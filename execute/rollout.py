...
# 两个考量方式：1. 与train时逻辑相同，看模仿的学习效果 2. 连续，看fitness
def rollout(opts,agent,epoch,tb_logger,tokenizer,testing=False):
    ...
    with tqdm(range(len(test_id)),desc='rollout') as pbar:
        ...
        for bat_id,id in enumerate(test_id):
            ...
            # log to tensorboard if needed，一个batch一张图，那就是八张图
            if need_log:
...
# with teacher
# 记录的cost，记录gap，记录outperform_ratio
...
def rollout_imitate(opts,task,agent,tokenizer,instances,testing):
    ...
    while not is_done:
        ...
        # ! change: rollout的时候没必要记录reward，outperform ratio可以通过最后的优化结果计算
